...
const NavItem: FC<NavItemProps> = ({ icon: Icon, text, onNavigate, route, isActive }) => (
  ...
  <li>
    <button className={`cursor-pointer flex items-center gap-4 text-lg w-full  p-2   group ${isActive ? "border-s-2 border-violet-500 text-violet-500" : "text-white "}`} onClick={() => onNavigate(route)} >
      <Icon />
...
const Sidebar = () => {
  ...
  const navItems: Omit<NavItemProps, 'onNavigate' | 'isActive'>[] = [
    ...
    { icon: TokensIcon, text: "My Tokens", route: "/tokens" },
